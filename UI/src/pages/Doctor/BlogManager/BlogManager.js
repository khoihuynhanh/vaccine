...
const BlogManagerDoctor = () => {
  ...
  const onSearchSubmit = async (data) => {
    await axiosCli()
      .get(`doctor/search-blog/${data.key}`)
      .then((res) => {
  ...
  const onUpdateSubmit = async (data) => {
    ...
    await axiosCli()
      .post(`doctor/update-blog/${idUpdate}`, allDataUpdate)
      .then((res) => {
        if (res.status == 200) {
          messageApi.open({
            type: "success",
  ...
  return (
    <div>
      ...
      <div className="flex">
        ...
        <Layout>
          <Content
            ...
            <Button className="float-right mb-5" onClick={showAdd}>
              Add New
            </Button>
            <div>
              <form
                className="space-y-1 md:space-y-6 mt-4 mb-10"
      ...
      <Modal
        ...
        <div>
          <div className="font-bold">{dataDel.name}</div>
        ...
        <form onSubmit={handleSubmit(onUpdateSubmit)}>
          ...
          <div className="mb-4">
            ...
            <textarea
              rows={5}
              {...register("sub_content")}
              defaultValue={dataUpdate.sub_content}
              placeholder="Tiêu Đề Phụ"
              name="sub_content"
              id="sub_content"
              className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5  dark:focus:ring-blue-500 dark:focus:border-blue-500"
