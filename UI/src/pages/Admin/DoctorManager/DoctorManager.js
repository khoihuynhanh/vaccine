...
const DoctorManager = () => {
  ...
  const onSearchSubmit = async (data) => {
    await axiosCli()
      .get(`admin/search-doctor/${data.key}`)
      .then((res) => {
  ...
  const showUpdate = async (e) => {
    ...
    setIdUpdate(id);
    await axiosCli()
      .get(`admin/get-data-doctor-by-id/${id}`)
      .then((res) => {
  ...
  const onUpdateSubmit = async (data) => {
    ...
    await axiosCli()
      .post(`admin/update-doctor/${idUpdate}`, dataSubmit)
      .then((res) => {
        if (res.status == 200) {
          messageApi.open({
            type: "success",
          ...
          <div className="mt-5">
            <div>
              ...
              {isChecked === true ? (
                ...
                <strong>Tài Khoản Bị Khoá</strong>
              )}
            </div>
