...
function AllVaccine() {
  ...
  const getDataVaccines = async () => {
    await axiosCli()
      .get(`user/all-vaccines?page=${currentPage}`)
      .then((res) => {
  ...
  const onSearchSubmit = async (data) => {
    await axiosCli()
      .get(`user/search-vaccine/${data.key}`)
      .then((res) => {
        ...
        {allDataVaccine?.map((data, index) => (
          <div key={index} className="w-full mb-4">
            <div className="w-full bg-white shadow-sm border hover:shadow-lg rounded-xl overflow-hidden">
              ...
              />
              <div className="p-4">
                <h2 style={{ minHeight: "84px" }} className="text-lg mb-2">
                  <strong>[{data.pro_code}]</strong> {data.pro_name}
                </h2>
              </div>
      ...
      <div className="mt-20">
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 gap-5 justify-left sm:flex mt-10 mb-10">
          {allDataBlog?.map((blog, index) => (
            <div key={index}>
              <div
                className="border w-full h-64 sm:w-72 bg-white shadow-md hover:shadow-lg rounded-xl overflow-hidden"
